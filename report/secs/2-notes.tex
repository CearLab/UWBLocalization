...
\begin{figure}[h!]
	...
	\begin{tikzpicture}[
		...
		\node[squarednode_black,minimum width = 2.5cm,minimum height = 1cm] (O) [xshift=-0cm,below=of P] {$f_o$};
            ...
            \node[squarednode_black,minimum width = 2.5cm,minimum height = 1cm] (UWB) [right=of J] {UWB};       
            %\node[squarednode_black,minimum width = 1cm,minimum height = 1cm] (E) [xshift=-2.3cm,yshift=-1.5cm,above=of J] {E};       
            % circles
            \node[draw,circle,minimum size=0.5cm,inner sep=0pt] (S1) [yshift=1.5cm,below=of P] {$+$};
            \node[draw,circle,minimum size=0.5cm,inner sep=0pt] (S2) [xshift=1.5cm,left=of UWB] {$+$};
		
		

		%Lines
            % Plant
		\draw[black, ultra thick, ->] (P.east) -- ([xshift=8.8cm]P.east) node [xshift=-3cm,above] (TextNode) {$x_t$} -- (UWB.north);
            % z meas            
            ...
            % y meas
            \draw[black, ultra thick, ->] (UWB.west) -- (S2.east) node [xshift=0.5cm,above] (TextNode) {$y^*_t$};
            \draw[black, ultra thick, ->] ([yshift=1cm]S2.north) -- (S2.north) node [yshift=0.5cm,right] (TextNode) {$v_t$};
            \draw[black, ultra thick, ->] (S2.west) -- (J.east) node [xshift=0.5cm,above] (TextNode) {$y_t$};
            % theta and switch
            ...
            % xhat
            \draw[black, ultra thick, ->] (O.west) -- ([xshift=-1cm]O.west) node [xshift=0.5cm,above] (TextNode) {$\hat{x}_t$};
		
		
		
	\end{tikzpicture}	
	\caption{\centering Hybrid observer}
\end{figure}