...
%Subjects to track
SubjectstoTrack={'CHEN'};%,'AlonTable'};
% Program options
...
while ishandle( MessageBox )
    ...
    fprintf( 'Subjects (%d):\n', SubjectCount );
    %   Checking names of subjects
    
%     SubNames=zeros(1,SubjectCount)
    for SubjectIndex = 1:SubjectCount
        SubNames= MyClient.GetSubjectName( SubjectIndex ).SubjectName;
        X(SubjectIndex) = cellstr(SubNames);
    end
    
    for jj = 1:length(SubjectstoTrack)  
        indname(jj)=find(strcmp(X,SubjectstoTrack(jj)));
    end
     
    
    
    for SubjectIndex = indname , %1:SubjectCount
        fprintf( '  Subject #%d\n', SubjectIndex - 1 );
